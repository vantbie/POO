...
# Clase Celda
# Representa una celda de la cuadrícula y valores A*
class Celda:
    def __init__(self, fila, col, tipo = 0):
        ...
        # para el algoritmo de búsqueda A*
        self._g = float('inf')
        self._h = 0
        self._f = float('inf')
    ...
    @property
    def coordenada(self):
        """Coordenada de la celda como tupla (fila, col)."""
        return (self._fila, self._col)

    @property
    def tipo(self):
        """Tipo numérico de la celda (0,1,2,3...)."""
        return self._tipo
    ...
    def resetar_obstaculo(self):
        """Restablece valores auxiliares usados por A* (padre, g, h, f)."""
        self._padre = None
        self._g = float('inf')
        self._h = 0
        self._f = float('inf')
...
# Clase Mapa
# Gestiona la cuadrícula y operaciones sobre ella
class Mapa:
    ...
    def obtener_vecinos(self, coord):
        """Devuelve lista de coordenadas accesibles (no obstáculo) en 4 direcciones."""
        f, c = coord
    ...
    def colocar_obstaculos_aleatorios(self, cantidad):
        """Coloca obstáculos aleatorios hasta alcanzar la cantidad (si es posible)."""
    ...
    def resetar_obstaculos(self):
        """Resetea los valores auxiliares de todas las celdas (usado por A*)."""
        for fila in self._matriz:
            for celda in fila:
                celda.resetar_obstaculo()

    def mostrar(self, opciones = None):
        """Imprime el mapa por consola. 'opciones' puede contener inicio, fin, ruta."""
        if opciones is None:
            opciones = {}
...
# Maneja el algoritmo seleccionado y guarda la última ruta
...
# Funciones de interacción (entrada/edición de obstáculos)
